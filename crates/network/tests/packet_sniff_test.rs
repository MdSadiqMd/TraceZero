...
async fn test_direct_request_exposes_plaintext() {
    ...
    println!("✓ Direct request exposes plaintext: {:?}", result.plaintext_found);
}
...
async fn test_tor_request_is_encrypted() {
    ...
    let encrypted_data: Vec<u8> = vec![
        ...
        0x8f, 0x2a, 0xb3, 0x91, 0x44, 0xc7, 0x5e, 0x12,
        0x9d, 0x6f, 0x88, 0x3c, 0xa1, 0x55, 0x7b, 0xe9,
        0x0c, 0x4d, 0x92, 0xf6, 0x28, 0x71, 0xba, 0x3e,
    ];
...
async fn test_compare_direct_vs_tor_traffic() {
    ...
    // Assertions
    assert!(direct_result.contains_plaintext, "Direct traffic should expose secrets");
    assert!(!tor_result.contains_plaintext, "Tor traffic should hide secrets");
...
async fn test_tor_hides_destination() {
    ...
    println!("Direct connection reveals destination: {}", direct_result.contains_plaintext);
    println!("Tor connection reveals destination: {}", tor_result.contains_plaintext);
...
#[tokio::test]
#[ignore = "Requires Tor to be running"]
async fn test_live_tor_connection() {
    ...
    let direct_client = TorHttpClient::new_direct().expect("Failed to create direct client");
    
    let tor_ip = tor_client.get_exit_ip().await.expect("Failed to get Tor IP");
    let direct_ip = direct_client.get_exit_ip().await.expect("Failed to get direct IP");
    ...
    let is_tor = tor_client.verify_tor_connection().await.expect("Failed to verify");
